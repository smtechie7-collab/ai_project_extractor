...
class Thermal58Renderer : BaseRenderer {
    ...
    override fun render(
        ...
        val titlePaint = (paints["title"] ?: TextPaint()).apply { textSize = 10f }
        val bodyPaint = (paints["body"] ?: TextPaint()).apply { textSize = 8f }
        val smallPaint = TextPaint(bodyPaint).apply { textSize = 6f }
        ...
        canvas.drawText("Inv: ${invoiceData.sale.invoiceNumber}", leftMargin, currentY, bodyPaint)
        currentY += bodyPaint.descent() - bodyPaint.ascent()
        canvas.drawText("Date: ${invoiceData.sale.date}", leftMargin, currentY, bodyPaint)
        currentY += bodyPaint.descent() - bodyPaint.ascent()

        // --- Dashed Line Separator ---
        val linePaint = Paint().apply { strokeWidth = 1f; pathEffect = android.graphics.DashPathEffect(floatArrayOf(2f, 2f), 0f) }
        currentY += 8
        ...
        currentY += bodyPaint.descent() - bodyPaint.ascent()
        canvas.drawText("Tax: ${invoiceData.totals.totalTax}", rightMargin, currentY, bodyPaint)
        currentY += bodyPaint.descent() - bodyPaint.ascent()
        
        titlePaint.textAlign = Paint.Align.RIGHT
        canvas.drawText("Total: ${invoiceData.totals.grandTotal}", rightMargin, currentY, titlePaint)
        currentY += titlePaint.descent() - titlePaint.ascent()
